...
INSTRUCTIONS = (
    "Gera projetos do CapCut Desktop no macOS: transcreve a fala, corta vídeo, coloca "
    "legendas sincronizadas e textos em momentos pré-determinados.\n\n"
    "FLUXO RECOMENDADO:\n"
    "  1. capcut.media.probe(sources=[...])  — descubra a duração real antes de montar\n"
    "  2. capcut.media.transcribe(source)    — LEIA o campo 'compact' para escolher\n"
    "  3. capcut.draft.create(name, 1080, 1920)\n"
    "  4. capcut.video.cut(source, keep=[[0,5],[12,18]], snap=\"speech\")\n"
    "     — declare os trechos que FICAM; snap evita cortar no meio da palavra\n"
    "  5. capcut.subtitle.from_transcript(draft_id, source)  — remapeia os tempos\n"
    "     sozinho. Só use capcut.subtitle.add quando os tempos JÁ forem da timeline\n"
    "     (por exemplo um SRT próprio) ou quando não houver corte.\n"
    "  6. capcut.text.add_many(items=[{text,timeline_start,duration}])\n"
    "     ou capcut.text.add para um texto só\n"
    "  7. capcut.draft.validate(draft_id)  — confira antes de gravar\n"
    "  8. capcut.draft.save(draft_id)\n"
    "  9. No CapCut, volte à página inicial para ele reler o disco.\n\n"
    "ESCOLHER OS TRECHOS é seu, não da ferramenta. Do que já foi medido aqui:\n"
    "  - o gancho vive nos 3 primeiros segundos: comece NA frase, não no meio dela;\n"
    "  - prefira trechos que começam e terminam em fronteira de sentença — o "
    "'compact' traz a pontuação justamente para isso;\n"
    "  - clipe vertical funciona entre 20 e 60 s;\n"
    "  - font_size 14-15 para título, 8 para legenda (medido na tela);\n"
    "  - não coloque texto depois do fim do vídeo: aparece sobre tela preta.\n\n"
    "ARMADILHA DE TEMPO: o transcript está em tempo da MÍDIA ORIGINAL; depois do "
    "corte a timeline tem outros tempos. Passar os blocos do transcribe direto para "
    "o subtitle.add gera legenda dessincronizada com JSON válido — nada acusa. Use "
    "o from_transcript, que remapeia, descarta o que caiu em trecho removido e "
    "conta quantos.\n\n"
